use iced_x86::{
    Decoder, DecoderOptions, Formatter, Instruction, MemorySize, Mnemonic, NasmFormatter, OpKind,
    Register, SpecializedFormatter, SpecializedFormatterTraitOptions,
};
...
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, mnemonic) == 0,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, mem_base) == 2,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, mem_index) == 3,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, mem_scale) == 4,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, stack_growth) == 5,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, regs) == 6,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, types) == 10,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, attributes) == 14,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, length) == 15,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, operand_count_visible) == 16,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, immediate) == 24,
    "invalid offset"
);
const _: () = assert!(
    offset_of!(MergenDisassembledInstructionBase, mem_disp) == 32,
    "invalid offset"
);
...
fn convert_type_to_mergen(instr: &Instruction, index: u32) -> OperandType {
    match instr.op_kind(index) {
        ...
        | OpKind::Immediate8to64 => OperandType::Immediate8,
...
fn analyze_instruction_bitfield(instr: &Instruction) -> u8 {
    ...
    for i in 0..op_count {
        match instr.op_kind(i) {
            OpKind::Immediate64 => {
                flags |= HAS_64BIT_IMM;
                if flags == (HAS_64BIT_IMM | IS_RELATIVE) {
                    return flags;
                }
            }
            OpKind::NearBranch16 | OpKind::NearBranch32 | OpKind::NearBranch64 => {
...
fn disassemble_instruction(instr: &Instruction) -> MergenDisassembledInstructionBase {
    ...
    MergenDisassembledInstructionBase {
        ...
        length: instr_len_u8,
        segment_prefix: instr.segment_prefix() as u8,
        is_broadcast: instr.is_broadcast(),
    }
}
#[inline(always)]
